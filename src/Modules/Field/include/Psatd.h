...
#pragma omp parallel for
	...
	class PSATDWithPoisson : public PSATD
	{
	public:
		PSATDWithPoisson(PSATDGrid* grid);

		void updateEB() override;
	...
	inline PSATDWithPoisson::PSATDWithPoisson(PSATDGrid* _grid) :
		PSATD(_grid)
	{
		updateDims();
		updateInternalDims();
	...
	inline void PSATDWithPoisson::updateEB()
	{
		const Int3 begin = updateComplexBAreaBegin;
		const Int3 end = updateComplexBAreaEnd;
		double dt = grid->dt / 2;
