...
#pragma omp parallel for
		...
		void pySetBxyz(py::function fBx, py::function fBy, py::function fBz)
		{
			...
			{
				FP3 cBx, cBy, cBz;
                cBx = convertCoords(this->BxPosition(i, j, k));
                cBy = convertCoords(this->ByPosition(i, j, k));
                cBz = convertCoords(this->BzPosition(i, j, k));
...
#pragma omp parallel for
	...
	private:
        ...
        virtual FP3 convertCoords(const FP3& coords) const {  // necessary for child grid with mapping
            return coords;
        }
    ...
    template<class pyGridType>
    class pyGridMapping : public pyGridType
    {
    public:

        pyGridMapping(const Int3 & _numInternalCells, FP _dt,
            const FP3 & minCoords, const FP3 & _steps, Mapping* mapping, bool ifCut = true) :
            pyGridType(_numInternalCells, _dt, minCoords, _steps), mapping(mapping), ifCut(ifCut) {}

        FP3 getE(const FP3& coords) const {
            bool status = false;
            FP3 inverseCoords = mapping->getInverseCoords(coords, &status);
            if (ifCut && !status)
                return FP3(0, 0, 0);
            return pyGridType::getE(inverseCoords);
        }
        ...
        FP3 getB(const FP3& coords) const {
            ...
            if (ifCut && !status)
                return FP3(0, 0, 0);
            return pyGridType::getB(inverseCoords);
        }
    ...
    private:
        ...
        FP3 convertCoords(const FP3& coords) const override {  // make inverse mapping
            return mapping->getDirectCoords(coords);
        }
    };

    typedef pyGridMapping<pyYeeGrid> pyYeeGridMapping;
    typedef pyGridMapping<pyPSTDGrid> pyPSTDGridMapping;
    typedef pyGridMapping<pyPSATDGrid> pyPSATDGridMapping;
}
